...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  const { provider, url_or_key, issues }: any = req.body;
  if (!issues || issues.length === 0) return res.status(400).json({ detail: "No issues provided." });

  const issuesText = issues.map((i: any) => `- ${i.key}: ${i.summary} (Type: ${i.type}, Status: ${i.status})`).join("\n");
  const prompt = `
...
Format everything clearly as Markdown.
  ...
  try {
    if (provider.toLowerCase() === 'ollama') {
      ...
      const availableModels: string[] = listResponse.ok ? (await listResponse.json() as any).models.map((m: any) => m.name) : [];
      ...
      if (response.ok) {
        let content: string = (await response.json() as any).response || "";
